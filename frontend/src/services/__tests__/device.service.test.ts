...
describe('Device Service - Complete CRUD Tests', () => {
  ...
  describe('DEVICE ASSIGNMENT - returnDevice', () => {
    it('should return device from user', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/devices/device-123/return'),
        expect.objectContaining({
          method: 'POST',
          body: JSON.stringify({ notes: 'Device returned in good condition' }),
        })
      );
    });
