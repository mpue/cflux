...
type SubTab = 'overview' | 'dashboard' | 'applicants' | 'equipment' | 'training' | 'jobs';
...
const OnboardingTab: React.FC<OnboardingTabProps> = ({ onUpdate }) => {
  ...
  const renderOverview = () => {
    const cards = [
      ...
      {
        title: 'Jobs',
        description: 'Stellen mit Beschreibungen für das Onboarding verwalten',
        icon: <Work sx={{ fontSize: 48 }} />,
        action: () => setActiveSubTab('jobs'),
        color: '#607d8b',
      },
    ];
