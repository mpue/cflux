// Dashboard Widgets - Centralized exports
export { default as TimeTrackingWidget } from './TimeTrackingWidget';
export { default as LoggedUsersWidget } from './LoggedUsersWidget';
export { default as RecentEntriesWidget } from './RecentEntriesWidget';
export { default as AbsenceRequestsWidget } from './AbsenceRequestsWidget';
export { default as SummaryWidget } from './SummaryWidget';
export { default as PendingApprovalsWidget } from './PendingApprovalsWidget';
export { default as MessagesWidget } from './MessagesWidget';
export { default as ClockWidget } from './ClockWidget';
export { default as BudgetWidget } from './BudgetWidget';
export { default as NotesWidget } from './NotesWidget';
export { default as WeatherWidget } from './WeatherWidget';
export { default as WidgetHeader } from './WidgetHeader';
export { default as WidgetSettingsModal } from './WidgetSettingsModal';
export { default as AddWidgetModal } from './AddWidgetModal';

export * from './types';

